    ...
    var body: some View {
        NavigationView {
            List {
                ...
                NavigationLink(tag: 3, selection: self.$selectedView, destination: { MangaMenuView(openedManga: $openedManga, openedChapter: $openedChapter) }, label: {
                        HStack {
...
struct ContentView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ContentView(openedManga: .constant(mangaVM[0]), openedChapter: .constant(mangaVM[0].chapters![0]))
    }
}
